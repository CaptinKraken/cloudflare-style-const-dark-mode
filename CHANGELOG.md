...
- Updates style-const to have a synced dark mode that works across SSR + astro + iframes

## 7.0.0-beta

### Major Changes

- **Cross-subdomain dark mode synchronization**: Added cookie-based dark mode sync across all `*.cloudflare.com` subdomains

  - Cookie `cf_dark_mode` on `.cloudflare.com` domain enables real-time sync between dashboard, developer docs, and other apps
  - Changes sync across tabs and iframes within 1 second

- **SSR support**: Added utilities for server-side rendered React apps

  - `initDarkMode()` - Explicit initialization for SSR apps (auto-initializes for CSR apps)
  - `getDarkModeFromRequest(request)` - Parse dark mode from request (checks `x-dark-mode` header first, then cookie)
  - `getDarkModeFromCookieHeader(cookieHeader)` - Parse dark mode from cookie header directly
  - `getInlineThemeScript(fallback)` - Generate inline script to prevent flash of unstyled content
  - `getDarkModeCookieName()` - Get the cookie name for reference

- **Automatic PostMessage sync**: For when cookies are blocked (Brave, privacy mode, browser settings)

  - PostMessage-based iframe sync now automatic with `initDarkMode()`
  - Provides instant parent↔iframe bidirectional sync
  - Security: Only accepts messages from `*.cloudflare.com` origins
  - No separate setup required - just works!

- **Framework support**: Works with React Router v7, TanStack Start, Next.js, Remix, Astro, and more

- **Backwards compatible**: Existing apps continue to work without changes. SSR apps can opt-in to new features.

- **Major simplification** - DRY and maintainable:

  - **PostMessage now automatic** - Built into `initDarkMode()`, no separate function needed
  - **Centralized sync manager** - All sync logic in single `DarkModeSyncManager` class
  - **Reduced API surface** - Single `initDarkMode()` call handles everything
  - **Instant dark mode toggle** - Removed transition suppression for simpler, more predictable behavior

- **Enhanced reliability**:

  - `initDarkMode()` returns cleanup function (prevents memory leaks)
  - Race condition protection (multiple initialization prevented)
  - `resetDarkMode()` - Test utility to reset state
  - `isLocalDevelopment()` - Check if running on localhost
  - `setDarkModeKey()` - Enhanced with Tailwind CSS example

- **Documentation**: Added comprehensive guides
  - `DARK_MODE_SYNC.md` - Complete technical guide with framework examples
  - `MIGRATION_GUIDE.md` - Upgrade instructions from 6.1.x
  - `COOKIE_CONSENT.md` - OneTrust configuration and legal compliance guide
  - `ADVANCED_USAGE.md` - Local dev, Tailwind, testing, edge cases, scalability

### Breaking Changes

- **Removed `modeTransitionTime`** from exported `theme` object
  - Was accessible as `theme.modeTransitionTime` or `variables.modeTransitionTime`
  - Removed transition suppression logic for simpler, instant dark mode toggle
  - No migration needed - this was an internal implementation detail

### Internal Changes

- **File reorganization**: Moved all dark mode logic to `src/darkMode.ts`
  - `utils.ts` is now renamed to `darkMode.ts`
  - Easier to find and maintain dark mode code in dedicated file
  - No API changes - all imports continue to work
